...
### Virtual Address with Memory-Pools
...
public:
    ...
    size_t unique_id {};
    t *p_not_found {};
    std::vector<pool_element_t<t>> loaded {};
public:
    pool(
        flags_t type_flags
    ) {
        this->type_flags = type_flags;
        this->p_not_found = new t {};
    }

    ~pool() {
        delete this->p_not_found;
    }
    ...
    at_t emplace_back() {
        t *p {new t {}};
        size_t index {this->loaded.size()};
        this->loaded.emplace_back() = {.p = p, .unique_id = ++this->unique_id };
        return {
            .type = this->type_flags,
    ...
    t *at(const at_t &at) {
        bool unique_id_conflict {};
        pool_element_t<t> &element;

        if (
            at.index >= this->loaded.size()
            ||
            (element = this->loaded.at(at.index)).p != nullptr
            ||
            element.should_be_dead
            ||
            (
                unique_id_conflict = (
                    element.unique_id
            ...
            if (unique_id_conflict) {
                size_t size {
                for (size_t it {}; it < query_element.size(); it++) {
                    pool_element_t<t> &query_element {this->loaded.at(it)};
                    if (query_element.unique_id == at.unique_id && query_element.p != nullptr) {
                        at.index = it;
                        return query_element.p;
                    }
                }
            }
            
            return this->p_not_found;
        }
...
int main() {
    pool<meow_t> meow_pool {type_element::MEOW};
    pool<moo_t> moo_pool {type_element::MOO};
    
    meow_t *p {meow_pool.at(meow_pool.emplace_back())};
    p->p_tag ="meow";
